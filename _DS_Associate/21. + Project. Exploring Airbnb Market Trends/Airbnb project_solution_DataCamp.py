...
# Read csv:
airbnb_price = pd.read_csv(os.path.join(os.path.dirname(__file__), 'data', 'airbnb_price.csv'))

# Read the TSV file using read_csv with tab delimiter and handle bad lines
airbnb_last_review = pd.read_csv(os.path.join(os.path.dirname(__file__), 'data', 'airbnb_last_review.tsv'), delimiter='\t', on_bad_lines='skip')

# Read the XLS file using read_excel
airbnb_room_type = pd.read_excel(os.path.join(os.path.dirname(__file__), 'data', 'airbnb_room_type.xlsx')) 


'''
# This works only on DataCamp platform
